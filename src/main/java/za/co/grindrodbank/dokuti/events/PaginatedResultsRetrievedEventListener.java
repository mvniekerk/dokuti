...
****************************************************/
package za.co.grindrodbank.dokuti.events;
 ...
 * 
 * https://gist.github.com/eugenp/1622997 &
 * https://dzone.com/articles/rest-pagination-spring <br>
...
public class PaginatedResultsRetrievedEventListener implements ApplicationListener<PaginatedResultsRetrievedEvent> {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(PaginatedResultsRetrievedEventListener.class);
	@Autowired
	...
	public void onApplicationEvent(PaginatedResultsRetrievedEvent event) {
		...
		pageResponse.setSize(tagsPage.getSize()); // The number items to include in a page of results. The page size for
													// the resultset
		pageResponse.setCurrent(tagsPage.getNumber()); // The position of the page in the paged resultset that is being
														// returned
		...
		UriComponentsBuilder lastBuilder = replacePageParams(original,
				PageRequest.of(tagsPage.getTotalPages() - 1, tagsPage.getSize()));
